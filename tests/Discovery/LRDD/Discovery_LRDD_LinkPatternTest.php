...
class Discovery_LRDD_LinkPatternTest extends Discovery_TestCase {
	...
	public function testComponentParsing() {
		...
		$components = Discovery_LRDD_LinkPattern::getComponents($resource);
	...
	public function testPattern() {
		...
		$template = new Discovery_LRDD_LinkPattern('http://example.org?q={%uri}');
		$this->assertEquals('http://example.org?q=http%3A%2F%2Fexample.com%2Fr%2F1%3Ff%3Dxml', $template->applyPattern($resource));

		$template = new Discovery_LRDD_LinkPattern('http://meta.{host}:8080{path}?{query}');
		$this->assertEquals('http://meta.example.com:8080/r/1?f=xml', $template->applyPattern($resource));

		$template = new Discovery_LRDD_LinkPattern('https://{authority}/v1{path}#{fragment}');
